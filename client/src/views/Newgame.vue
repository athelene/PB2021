<template class="appBackground">
  <div class="newgame appBackground text-center" v-if="user.UserID > 0">
    <h1 class="ml-5 headingColor">New Game</h1>
        <v-card 
...
//        this.getGameList();
    ...
    async getHourStatus() {
      console.log(' starting getHourStatus, this.courts[this.selectedCourt].CourtID ', this.courts[this.selectedCourt].CourtID)
      await EventService.getHourStatus(this.gameDate, this.courts[this.selectedCourt].CourtID)
      .then(
        (hourReturn => {
          this.hrBlx = hourReturn
            console.log('gamedate and hhrBlx are: ', this.gameDate, this.hrBlx)
            console.log('this.hrBlx.length is: ', this.hrBlx.length)
        })
    ...
    async saveGame() {
     ...
     console.log('this.gameStart, gameEndTime, this.gameDate, this.groupID, this.selectedCourt: ', gdt, gameEndTime, this.groupID, this.selectedCourt)
        await EventService.saveGame(gdt, gameEndTime, this.groupID, this.courtToAdd)
      .then((insertResult) => {
...
//        this.groupID = '';
    ...
    async gameMessage(gdt, gameEndTime, groupID, courtToAdd) {
      await EventService.gameMessage(gdt, gameEndTime, groupID, courtToAdd, 'newsingle')
