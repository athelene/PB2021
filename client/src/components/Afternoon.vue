...
<div class="">
  ...
  <h3 v-if="earliestTime !== '' && earliestTime !== null && earliestTime !== undefined"> The earliest your group can start is: {{earliestTime}} </h3>        <v-btn v-if="!available"
          color="blue"
          dark
          @click="setAvailable()"
        >
...
export default {
  ...
  methods: {
      ...
      async saveTime(){
        ...
        await EventService.setMyTime(this.user.UserID, this.selectedTime, this.gameDate, 'P')
      .then(
        (() => {
      ...
      .then(
        ((earliest) => {
          ...
          this.earliestTime = earliest.recordsets[0][0].StartTime2
            console.log('earliestTime rec from ES P is : ',  this.earliestTime)
        } else {
            this.earliestTime = '';
        }
        })
